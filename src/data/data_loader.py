...
class KLineDataset(Dataset):
    def __init__(self, db_manager, inst_id: str, 
        ...
        self.mode = mode
        self.lookback = data_config['lookback']
        self.forecast = data_config['forecast']
        self.split_ratio = data_config['split_ratio']
        self.thresholds = [-0.5, -0.2, 0, 0.2, 0.5] if data_config['thresholds'] is None else data_config['thresholds']
        self.use_label_smoothing = data_config['use_label_smoothing']
        ...
        # 数据集划分
        self.train_set, self.val_set, self.test_set = self._split_dataset(self.split_ratio)
    ...
    def _preprocess_data(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for i in range(len(prices) - self.lookback - self.forecast):
            ...
            if self.mode == 'regression':
                ...
                targets.append(target.astype(np.float32))  # 确保是float32类型
            else:
                # 计算实际价格变化百分比
                current_price = prices[i+self.lookback-1]
                future_prices = prices[i+self.lookback:i+self.lookback+self.forecast]
                ...
                if self.use_label_smoothing:
                    target = self._get_smoothed_labels(target)  # 返回的是概率分布
                    targets.append(target.astype(np.float32))  # 使用float32
                else:
                    targets.append(target.astype(np.float32))  # 普通分类使用整数
        ...
        if self.mode == 'regression':
            ...
            if self.use_label_smoothing:
                targets_tensor = torch.stack([torch.FloatTensor(t) for t in targets])  # 平滑标签用FloatTensor
                print("!!!!!!!!!!!!", targets_tensor.shape)
            else:
                targets_tensor = torch.LongTensor(np.array(targets))  # 普通分类用LongTensor
    ...
    def _get_smoothed_labels(self, class_idx):
        ...
        smooth_val = 0.1  # 平滑系数
        labels = np.full(num_classes, smooth_val/(num_classes-1), dtype=np.float32)  # 直接用 NumPy
        labels[class_idx] = 1.0 - smooth_val
        return labels  # 返回 NumPy 数组
